...
// ─── Definición de rutas con íconos y subtítulo ────────────────────────────
...
const navLinks = [
  ...
  { to: '/ingredientes',  label: 'Ingredientes', Icon: IconIngredientes },
  { to: '/perfil',        label: 'Perfil',       Icon: IconPerfil },
  { to: '/login',         label: 'Login',        Icon: IconLogin },
]

// Divisor entre grupos de nav
const Divider = () => (
...
function getNavClass({ isActive }: NavLinkState): string {
  ...
  if (isActive) {
    return `${base} text-white nav-active-glow`
  }
...
function App() {
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'linear-gradient(160deg, #f0f4ff 0%, #f8faff 45%, #eff6ff 100%)' }}>
      ...
      >
        <nav className="flex justify-between items-center max-w-7xl mx-auto px-6 h-14 gap-6">
